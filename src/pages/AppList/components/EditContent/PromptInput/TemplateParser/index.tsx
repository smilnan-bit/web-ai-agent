...
// @ts-ignore - Module resolution issue with exports subpath
import type { SelectionEnlargerSpec } from '@flowgram.ai/coze-editor';
...
export class TemplateParser {
  ...
  constructor(props: { mark: 'InputSlot' }) {
  ...
  getSelectionInMarkNodeRange(range: { from: number; to: number }, state: EditorState): MarkRangeInfo | null {
    return this.getPostionInMarkNodeRange(range.from, state) && this.getPostionInMarkNodeRange(range.to, state);
  }

  getPostionInMarkNodeRange(postion: number, state: EditorState) {
    const markRangeInfo = state
      .field(this.markInfoField)
      .marks.find((rangeInfo) => rangeInfo.from < postion && postion < rangeInfo.to);
    if (markRangeInfo) {
  ...
  generateOpenTemplateByData(data: { [key: string]: string }) {
    ...
    const text = keys.map((key, index) => `${key}="${values[index]}"`).join(' ');
  ...
  insertTemplateByRange(editor: EditorAPI, template: string, range: { from: number; to: number }) {
    const { from, to } = range;
    editor.$view.dispatch({
      changes: {
        from,
  ...
  extractTemplateContent(template: string) {
    ...
    const regex = new RegExp(`\\{#${this.mark}\\s+[^#]+#\\}|\\{#\\/${this.mark}#\\}`, 'g');
  ...
  /**
   * Convert template syntax to simplified format
   * {#mark ...#}content{#/mark#} -> {content}
   * Example: {#InputSlot placeholder="xxx"#}name{#/InputSlot#} -> {name}
   */
  convertTemplateToSimplifiedFormat(text: string): string {
    const templateRegex = new RegExp(`(\\{#${this.mark}\\s+[^#]+#\\})([\\s\\S]*?)(\\{#\\/${this.mark}#\\})`, 'g');

    return text.replace(templateRegex, (_match, _openTag, content) => {
      // Extract content and wrap it in curly braces
      return `{${content}}`;
    });
  }
}
