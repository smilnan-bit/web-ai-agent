...
namespace UniqueWorkflowUtils {
  ...
  export const generateNodeReplaceMap = (
    ...
    nodeIds.forEach((id) => {
      if (isUniqueId(id)) {
        ...
        let newId: string;
        do {
          newId = nanoid(); // generate new id until unique - 生成新ID直到唯一
  ...
  /**
   * regenerate edge ids in workflow json - 重新生成工作流JSON中的边ID
   * Handles both top-level edges and edges in sub-canvas (nested blocks)
   * - 处理顶层边和子画布（嵌套块）中的边
   * notice: this method has side effects, it will modify the input json
   * - 注意：此方法有副作用，会修改输入的json
   */
  export const regenerateEdgeIds = (json: WorkflowJSON): WorkflowJSON => {
    // Helper function to regenerate edgeId in lineData - 辅助函数：重新生成 lineData 中的 edgeId
    const regenerateEdgeInLineData = (edge: any) => {
      edge.data = { edgeId: nanoid() };
    };

    // Process top-level edges - 处理顶层边
    json.edges?.forEach(regenerateEdgeInLineData);

    // Process edges in nested blocks (sub-canvas) - 处理嵌套块中的边（子画布）
    const processNodeEdges = (node: WorkflowNodeJSON) => {
      // Check if node has edges (sub-canvas) - 检查节点是否有边（子画布）
      (node as any).edges?.forEach(regenerateEdgeInLineData);
      // Recursively process child blocks - 递归处理子块
      node.blocks?.forEach(processNodeEdges);
    };
    json.nodes.forEach(processNodeEdges);
...
export const generateUniqueWorkflow = (params: {
  ...
  const updatedJson = UniqueWorkflowUtils.replaceNodeId(json, nodeReplaceMap); // replace all node ids - 替换所有节点ID
  return UniqueWorkflowUtils.regenerateEdgeIds(updatedJson); // regenerate all edge ids - 重新生成所有边ID
};
