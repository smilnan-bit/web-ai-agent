...
export class CopyShortcut implements ShortcutsHandler {
  ...
  private getNodeJSONs(nodes: WorkflowNodeEntity[]): WorkflowNodeJSON[] {
    const nodeJSONs = nodes.map((node) =>
      node.flowNodeType === FlowNodeBaseType.GROUP ? this.getGroupNodeJSON(node) : this.document.toNodeJSON(node),
    );
  ...
  private notifySuccess(): void {
    const startEndNodeTypes: WorkflowNodeType[] = [WorkflowNodeType.Start, WorkflowNodeType.End];
    if (this.selectedNodes.some((node) => startEndNodeTypes.includes(node.flowNodeType as WorkflowNodeType))) {
      message.warning({
        content: '开始/结束节点不能被复制，其他节点已复制到剪贴板',
      });
      return;
    }
    message.success({
